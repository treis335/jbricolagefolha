...
function buildAnnualData(collaborators: any[], year: number) {
  const months: MonthSummary[] = Array.from({ length: 12 }, (_, i) => {
    ...
    return { month: `${year}-${m}`, label: new Date(year, i, 1).toLocaleDateString("pt-PT", { month: "short" }), horas: 0, custo: 0, pago: 0, pendente: 0, nAtivos: 0 }
  })
  ...
  collaborators.filter(c => c.ativo !== false).forEach(collab => {
    ...
    months.forEach(ms => {
      ...
      if (!md) return
      ms.horas += md.hours; ms.custo += md.cost; ms.pago += md.paid; ms.pendente += md.pending
      if (md.hours > 0) { ms.nAtivos++; mesesAtivos++ }
      totalH += md.hours; totalC += md.cost; totalP += md.paid; totalPend += md.pending
    })
...
function exportCSV(months: MonthSummary[], collabs: CollabAnnual[], year: number) {
  ...
  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `relatorio-anual-${year}.csv`; a.click(); URL.revokeObjectURL(a.href)
}
...
async function exportPDF(months: MonthSummary[], collabs: CollabAnnual[], year: number) {
  ...
  doc.setFontSize(18); doc.setFont("helvetica", "bold"); doc.text(`JBricolage — Relatório Anual ${year}`, 14, 18)
  doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(100)
  doc.text(`Gerado em: ${new Date().toLocaleDateString("pt-PT")}`, 14, 25)
  autoTable(doc, {
    startY: 30,
    head: [["Mês", "Horas", "Custo", "Pago", "Pendente", "Ativos"]],
    body: months.filter(m => m.horas > 0 || m.pago > 0).map(m => [m.label, `${(Number(m.horas) || 0).toFixed(1)}h`, `${(Number(m.custo) || 0).toFixed(2)}€`, `${(Number(m.pago) || 0).toFixed(2)}€`, `${(Number(m.pendente) || 0).toFixed(2)}€`, m.nAtivos]),
  ...
  doc.setFontSize(12); doc.setFont("helvetica", "bold"); doc.setTextColor(0); doc.text("Ranking de Colaboradores", 14, y2)
  autoTable(doc, {
    startY: y2 + 5,
    ...
    styles: { fontSize: 9, cellPadding: 2.5 }, headStyles: { fillColor: [15, 23, 42], textColor: 255, fontStyle: "bold" }, alternateRowStyles: { fillColor: [248, 250, 252] },
  })
...
export function AnnualReportModal({ open, onClose, collaborators = [] }: { open: boolean; onClose: () => void; collaborators?: import("@/hooks/useCollaborators").Collaborator[] }) {
  ...
  async function doExport(type: "csv" | "pdf") {
    setExp(type)
    try { if (type === "csv") exportCSV(months, collabs, year); else await exportPDF(months, collabs, year) }
    finally { setExp(null) }
  }
        ...
        <div className="flex border-b shrink-0 px-5">
          {(["mensal", "colaboradores"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={cn("py-2.5 px-3 text-xs font-semibold border-b-2 transition-colors",
                ...
                <thead className="sticky top-0 bg-muted/60 backdrop-blur-sm">
                  <tr className="border-b border-border/30">
                    {["Mês", "Horas", "Custo", "Pago", "Pendente", "Ativos"].map(h => (
                      <th key={h} className={cn("text-[11px] font-bold uppercase tracking-wide text-muted-foreground py-2.5 px-3", h === "Mês" ? "text-left pl-5" : "text-right", h === "Ativos" && "pr-5")}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {months.map((m, i) => (
                    <tr key={m.month} className={cn("border-b border-border/15 hover:bg-muted/20", i % 2 === 1 && "bg-muted/10", m.horas === 0 && m.pago === 0 && "opacity-30")}>
                      ...
                      <td className="py-2.5 px-3 text-right tabular-nums">
                        {m.pendente > 0 ? <span className="text-amber-600 dark:text-amber-400 font-semibold">{fmt(m.pendente)}</span> : m.custo > 0 ? <span className="text-emerald-500 text-xs">✓ Pago</span> : "—"}
                      </td>
                ...
                <thead className="sticky top-0 bg-muted/60 backdrop-blur-sm">
                  <tr className="border-b border-border/30">
                    {["#", "Colaborador", "Total Horas", "Custo Total", "Pago", "Pendente", "Meses Ativos"].map(h => (
                      <th key={h} className={cn("text-[11px] font-bold uppercase tracking-wide text-muted-foreground py-2.5 px-3", h === "Colaborador" || h === "#" ? "text-left" : h === "Meses Ativos" ? "text-right pr-5" : "text-right")}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {collabs.map((c, i) => (
                    <tr key={c.name} className={cn("border-b border-border/15 hover:bg-muted/20", i % 2 === 1 && "bg-muted/10")}>
                      ...
                      <td className="py-2.5 px-3 text-right tabular-nums">
                        {c.totalPend > 0 ? <span className="text-amber-600 dark:text-amber-400 font-semibold">{fmt(c.totalPend)}</span> : <span className="text-emerald-500 text-xs">✓</span>}
                      </td>
          ...
          <div className="shrink-0 px-5 py-4 border-t bg-background flex items-center justify-between gap-3">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <button onClick={() => doExport("csv")} disabled={!!exporting}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold transition-all disabled:opacity-50">
                <FileSpreadsheet className="h-4 w-4" />{exporting === "csv" ? "A exportar…" : "Excel (.csv)"}
              </button>
              <button onClick={() => doExport("pdf")} disabled={!!exporting}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-all disabled:opacity-50">
