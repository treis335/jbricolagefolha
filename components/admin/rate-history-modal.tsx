  ...
  collaborators.forEach(c => {
    ...
    hist.forEach(h => {
      rows.push({
        ...
        taxaAnterior: h.taxaAnterior != null ? Number(h.taxaAnterior) : null,
        data:         h.data || "",
        dataVigencia: h.dataVigencia || "",
        alteradoPor:  h.alteradoPor || "—",
        motivo:       h.motivo || "",
        delta:        h.taxaAnterior != null ? (Number(h.taxa) || 0) - (Number(h.taxaAnterior) || 0) : 0,
      })
    })
  })
...
async function exportPDF(rows: HistoryRow[], collaborators: any[]) {
  ...
  doc.text(`Gerado em: ${new Date().toLocaleDateString("pt-PT")} · ${rows.length} alterações`, 14, 26)
  autoTable(doc, {
    startY: 32,
    head: [["Data Vigência", "Colaborador", "Anterior", "Nova Taxa", "Δ", "Alterado Por", "Motivo"]],
    body: rows.map(r => [
      r.dataVigencia || (r.data ? new Date(r.data).toLocaleDateString("pt-PT") : "—"),
      r.collaborator,
    ...
    columnStyles: {
      0: { cellWidth: 28 }, 1: { cellWidth: 40 }, 2: { halign: "right", cellWidth: 26 },
      3: { halign: "right", cellWidth: 24 }, 4: { halign: "right", cellWidth: 20 },
      5: { cellWidth: 30 }, 6: { cellWidth: "auto" as any },
    },
    didParseCell: (data) => {
  ...
  if (finalY < 190) {
    ...
    autoTable(doc, {
      ...
      body: collaborators
        .filter(c => c.ativo !== false && c.currentRate > 0)
        .sort((a, b) => b.currentRate - a.currentRate)
        .map(c => [c.name, `${(Number(c.currentRate) || 0).toFixed(2)}€/h`, (c.rateHistory || []).length]),
...
export function RateHistoryModal({
  open, onClose, collaborators = [],
}: {
  open: boolean
  onClose: () => void
  collaborators?: import("@/hooks/useCollaborators").Collaborator[]
}) {
          ...
          {rows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
              ...
              <p className="text-sm font-medium">
                {search ? "Sem resultados para a pesquisa" : "Sem histórico de taxas registado"}
              </p>
            </div>
          ) : (
                  ...
                  <tr className="border-b border-border/30">
                    {["Data", "Colaborador", "Anterior", "Nova Taxa", "Δ", "Alterado Por", "Motivo"].map(h => (
                      <th key={h} className={cn(
                        ...
                        h === "Data" || h === "Colaborador" || h === "Alterado Por" || h === "Motivo"
                  ...
                  {rows.map((r, i) => (
                    <tr key={i} className={cn("border-b border-border/15 hover:bg-muted/20", i % 2 === 1 && "bg-muted/10")}>
                      <td className="py-2.5 pl-5 pr-3 text-[11px] text-muted-foreground tabular-nums whitespace-nowrap">
                        {r.dataVigencia
                          ? new Date(r.dataVigencia + "T00:00:00").toLocaleDateString("pt-PT")
                          : r.data
                      ...
                      <td className="py-2.5 px-3 text-right tabular-nums">
                        {r.taxaAnterior == null ? (
                          <span className="text-[10px] text-muted-foreground/50 bg-muted/50 px-1.5 py-0.5 rounded">Inicial</span>
                        ) : r.delta > 0 ? (
